...
SEXP semsim(SEXP anotaciones, SEXP sic){
	...
	//Armamos el vector de nombres de categorías
	...
	for(categoria1 = 0; categoria1 < len_anotaciones; ++categoria1){
		...
		for(categoria2 = categoria1; categoria2 < len_anotaciones; ++categoria2){
			...
			for(l = 0; l < len; ++l){
				for(m = 0; m < len2; ++m){
					if(strcmp(padres[l], CHAR(STRING_ELT(VECTOR_ELT(anotaciones, categoria2), m))) == 0){
						iic = getListElement(nombres, padres[l], len_nombres);
